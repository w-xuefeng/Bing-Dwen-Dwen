const pathCounter = 628;
let drawPath = 0;

function drawCanvas(canvas, targetFrame, resizing) {
  ...
  function drawNextPath(onCompleted) {
    drawPath++;
  ...
  return new Promise(drawNextPath)
}
...
StyleKitName = {
  ...
  resizeCanvas: function(mainCanvas) {
  ...
  play: function(canvas, targetFrame = makeRect(0, 0, 1068, 1262), resizing = 'aspectfit') {
    if (this.playing) {
      return;
    }
    this.reset(canvas);
    ...
    this.drawCanvas(canvas, targetFrame, resizing).then(() => {
      this.playing = false;
      console.log('playing completed!')
    })
  }
}

function getDom(selector, parent = document) {
  return parent.querySelector(selector)
}

function init(btnSelector = 'button.btn', canvasId = 'canvas') {
  StyleKitName.resizeCanvas(getDom(`#${canvasId}`));
  window.addEventListener('resize',function() {
    StyleKitName.resizeCanvas(getDom(`#${canvasId}`));
  })
  getDom(btnSelector).addEventListener('click',function() {
    StyleKitName.play(canvasId);
  })
}
